...
The guest reaches two host services, both on the bridge address: ananke's OpenAI port, and redlib. redlib listens on `0.0.0.0` already, so the accept in `default.nix` is the whole of the grant; its port is read from `services.redlib.port` rather than repeated. redlib is a read-only Reddit front-end with no account attached, so reaching it costs the sandbox nothing that the guest's unrestricted internet egress does not already cost. Knowing the address is separate from being able to reach it: the agent is told about `http://10.100.0.1:10000` through its profile, which is part of the manual enrolment. The profile must also say which tool to use. `web_extract` cannot reach redlib and no host-side change will make it: every extract-capable provider (Tavily, Firecrawl, Exa, Parallel) fetches the URL from its own network, and a bridge address does not exist there. The browser tools drive a local Chromium in the guest and do reach it; `terminal` does too, and is the better choice for anything recurring, since redlib serves RSS feeds that are cheaper and more stable than a page snapshot.
