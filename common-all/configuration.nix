{ config, pkgs, lib, ... }:
{
  options.mainUser = lib.mkOption {
    type = lib.types.str;
    default = "philpax";
    description = "The primary human user account: the one with sudo, the interactive desktop session, and the personal files under /home/<name>. Modules derive home paths from this rather than hardcoding them.";
  };

  imports =
    [
      /etc/nixos/hardware-configuration.nix
      (import ./services { inherit config pkgs; })
      (import ./programs { inherit config pkgs; })
    ];

  config = {
    system.autoUpgrade.enable = true;
    nix.settings.experimental-features = [ "nix-command" "flakes" ];
    nixpkgs.config.allowUnfree = true;
    nixpkgs.overlays = [ (import ./overlays/helix-steel.nix) ];

    boot.kernelPackages =
      if config.boot.zfs.enabled
      then pkgs.linuxPackages
      else pkgs.linuxPackages_latest;

    time.timeZone = lib.mkDefault "Europe/Stockholm";
    i18n.defaultLocale = "en_AU.UTF-8";
    i18n.supportedLocales = ["en_AU.UTF-8/UTF-8"];
    i18n.extraLocaleSettings = {
      LC_ALL = "en_AU.UTF-8";
      LC_CTYPE = "en_AU.UTF-8";
      LC_ADDRESS = "en_AU.UTF-8";
      LC_IDENTIFICATION = "en_AU.UTF-8";
      LC_MEASUREMENT = "en_AU.UTF-8";
      LC_MONETARY = "en_AU.UTF-8";
      LC_NAME = "en_AU.UTF-8";
      LC_NUMERIC = "en_AU.UTF-8";
      LC_PAPER = "en_AU.UTF-8";
      LC_TELEPHONE = "en_AU.UTF-8";
      LC_TIME = "en_AU.UTF-8";
    };

    networking.nameservers = [ "1.1.1.1" "1.0.0.1" "8.8.8.8" "8.8.4.4" ];

    users.groups.editabledata = {};

    users.users.philpax = {
      isNormalUser = true;
      description = "philpax";
      shell = pkgs.fish;
      # Rootless Podman maps container uids into this range and needs a
      # persistent user manager for long-lived rootless services.
      autoSubUidGidRange = true;
      linger = true;
      # Do not add the Docker or Podman API groups here: either socket grants
      # control equivalent to root for the corresponding engine.
      extraGroups = [ "networkmanager" "wheel" "libvirtd" "kvm" "dialout" "plugdev" "uucp" "ai" "editabledata" "input" "uinput" ];
      packages = with pkgs; [];
      openssh.authorizedKeys.keys = [
        "ssh-rsa  me@philpax.me"
      ];
    };
  };
}
