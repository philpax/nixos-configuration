...
| `update-ai.py` | Regenerate the Maki and Polytoken provider configs (`providers.toml`, `config.yaml`) from the ananke model definitions in `mindgame/` and `redline/`. Run before `sync.sh` when the served models change: `uv run update-ai.py`. `--check` exits non-zero if the generated files have drifted from their templates. |
