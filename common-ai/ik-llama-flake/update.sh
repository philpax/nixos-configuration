#!/usr/bin/env bash
# Update the pinned ik_llama.cpp rev and rebuild the flake.
#
# Usage: ./update.sh [ref]
#   ref: a branch, tag, or commit sha of ikawrakow/ik_llama.cpp (default: main)
#
...
old=$(sed -nE 's|.*"github:ikawrakow/ik_llama\.cpp/([0-9a-f]{40})".*|\1|p' flake.nix)
if [[ -z $old ]]; then
...
sed -i "s|github:ikawrakow/ik_llama\.cpp/$old|github:ikawrakow/ik_llama.cpp/$rev|" flake.nix

nix flake update ik-llama-cpp
