...
2. **Quiz the user** — the initial description is rarely enough. Before drafting, ask follow-up questions using the `ask_user_question` tool. Ask about anything that's missing from this checklist:
   ...
   Don't ask all of these mechanically — only ask about what the user's description and the codebase context don't already answer. Batch the questions in a single `ask_user_question` tool call (up to four questions) so the user can answer them at once. Each question supports free-text answers, so don't add a separate "other" option.
...
> Introduced in `a1b2c3d` — the hook skipped the counter reset on the
> second pass, leaving the overflow count unreset.
...
- Keep titles concise and specific, and prefer one clause over more: name the component and the symptom in a single phrase (e.g. "Login form ignores submit on second click" not "login bug"). If you can't express it in one clause, the issue may be too broad — consider splitting. Do not append a second explanatory clause after an em-dash or comma — the title names the problem; the body explains it. ✅ "Frobnicator's encabulator requires tuning" ❌ "Frobnicator's encabulator requires tuning — maximize efficiency in system"
